...
- **Public starting selection:** `src/data/featured-imports.json` is an explicit list of Instagram import IDs permitted to appear while their source files remain drafts. The current selection is 18 cooking entries. The four homepage spotlights are chosen in `src/data/spotlight-imports.json`. Remove an ID to hide it again. This override never publishes entries marked `needsReview` or `archivedOnInstagram`. Publishing by `draft: false` is an explicit author decision and can include archived entries.
- **Journal:** starts with an honest empty state and one unpublished starter draft. Add your own words; no fictional personal stories are public.
